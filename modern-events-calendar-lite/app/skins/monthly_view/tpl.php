...
if($this->next_previous_button)
{
    ...
    // adventistai.lt: jump-a-year-back + Today, using the same
    // .mec-load-month mechanism MEC's own JS already (re)binds after
    // every month load, so no extra JS is needed for these to work.
    $_1year_before = strtotime('-1 year', $current_month_time);
    $_1year_after  = strtotime('+1 year', $current_month_time);
    $today_ts      = current_time('timestamp');
    $navigator_html .= '<button type="button" class="mec-ymtabs-jump mec-load-month" data-mec-year="'.date('Y', $_1year_before).'" data-mec-month="'.date('m', $_1year_before).'">&laquo;</button>';
    $navigator_html .= '<button type="button" class="mec-ymtabs-today mec-load-month" data-mec-year="'.date('Y', $today_ts).'" data-mec-month="'.date('m', $today_ts).'">'.esc_html__('Today', 'modern-events-calendar-lite').'</button>';
    ...
    // adventistai.lt: jump-a-year-forward
    $navigator_html .= '<button type="button" class="mec-ymtabs-jump mec-load-month" data-mec-year="'.date('Y', $_1year_after).'" data-mec-month="'.date('m', $_1year_after).'">&raquo;</button>';
}
...
?>
<?php
// adventistai.lt: build the small native navigator's markup here so
// it can be handed to ymtabs_render_bar() and rendered as the bottom
// row of that same panel, instead of appearing further down inside
// .mec-calendar-side as a visually separate box. Same id/class as
// before, so every existing JS selector that finds it by id or by
// ".mec-skin-monthly-view-month-navigator-container" within
// #mec_skin_<id> still works — only its position in the DOM moved.
$navigator_container_html = '';
if($this->next_previous_button)
{
    $navigator_container_html = '<div class="mec-skin-monthly-view-month-navigator-container"><div class="mec-month-navigator" id="mec_month_navigator_'.esc_attr($this->id).'_'.date('Ym', $current_month_time).'">'.$navigator_html.'</div></div>';
}
?>
<div id="mec_skin_<?php echo esc_attr($this->id); ?>" class="mec-wrap <?php echo esc_attr($event_colorskin . ' ' . $this->html_class . ' ' . $set_dark); ?>">

    <?php echo MEC_kses::full($this->ymtabs_render_bar($navigator_container_html)); ?>
