...
class MEC_skin_monthly_view extends MEC_skins
{
    ...
    public function actions()
    {
        ...
        $this->factory->action('wp_ajax_mec_ymtabs_get_year', array($this, 'ymtabs_ajax_get_year'));
        $this->factory->action('wp_ajax_nopriv_mec_ymtabs_get_year', array($this, 'ymtabs_ajax_get_year'));
    }
     ...
     * A year-tab strip with per-month event counts on top of this skin,
     * plus a Today/jump-year enhancement on the existing month navigator.
     ...
     *  Lithuanian dates/calendars actually abbreviate them). */
    public function ymtabs_lithuanian_month_abbr()
    {
        return array(
    ...
    public function ymtabs_short_month_label($month_num, $full_label)
    {
        $abbr = apply_filters('mec_ymtabs_short_month_names', null);

        if(!is_array($abbr))
        {
            $abbr = (strpos(get_locale(), 'lt') === 0) ? $this->ymtabs_lithuanian_month_abbr() : array();
        }
     ...
     *  indexed columns on #__mec_dates. */
    public function ymtabs_counts_for_year($year)
    {
        global $wpdb;
    ...
    public function ymtabs_render_bar($extra_html = '')
    {
        $start = strtotime($this->start_date);
        $year  = (int) wp_date('Y', $start);
        $month = (int) wp_date('n', $start);

        $atts_qs = http_build_query(array('atts' => $this->atts), '', '&');

        ob_start();
        ?>
        <div class="mec-ymtabs" data-mec-id="<?php echo esc_attr($this->id); ?>" data-mec-atts="<?php echo esc_attr($atts_qs); ?>" data-ajax-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>">
            ...
            <div class="mec-ymtabs-months">
                <?php echo $this->ymtabs_render_months_row($year, $year, $month); ?>
            </div>

            <?php
            // adventistai.lt: the small native "« ‹ Today › »" navigator
            // is passed in here (rather than staying in its original spot
            // further down the template) so the two render as one visually
            // continuous panel instead of two separate boxes with a gap.
            echo MEC_kses::page($extra_html);
            ?>
    ...
    public function ymtabs_render_months_row($year, $active_year, $active_month)
    {
        $counts = $this->ymtabs_counts_for_year($year);
        ...
        for($m = 1; $m <= 12; $m++)
        {
            ...
            $short     = $this->ymtabs_short_month_label($m, $label);
    ...
    public function ymtabs_ajax_get_year()
    {
        // adventistai.lt: force the SITE's locale for this AJAX request —
        // see the matching note in weekly_view.php's load_month() for why.
        $site_locale = get_option('WPLANG');
        ...
        wp_send_json(array(
            'year' => $year,
            'html' => $this->ymtabs_render_months_row($year, $active_year, $active_month),
        ));
    }
}
