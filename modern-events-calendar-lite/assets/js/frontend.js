...
/* =============================================================
 * adventistai.lt customization: year-tab bar for Monthly View.
 * Month-tab clicks own their navigation independently (goToMonth,
 * bound via delegation below) rather than relying on MEC's own
 * .mec-load-month + initMonthNavigator() mechanism: that mechanism
 * only (re)binds clicks on elements present at the moment it last
 * ran (page load, or right after MEC's own AJAX month-swap), so
 * tabs inserted by our own year-swap AJAX (loadYear, below) never
 * got a working click handler under the old approach and were dead
 * until the calendar happened to reload for some other reason.
 * Delegated binding sidesteps that: it works for every tab, present
...
(function ($) {
    ...
    function setActiveTab($bar, year, month) {
        var mm = ('0' + month).slice(-2);
        $bar.find('.mec-ymtabs-item').removeClass('mec-ymtabs-item-active');
        $bar.find('.mec-ymtabs-item[data-mec-year="' + year + '"][data-mec-month="' + mm + '"]').addClass('mec-ymtabs-item-active');
    }

    function loadYear($bar, year, activeYear, activeMonth) {
        $bar.addClass('mec-ymtabs-loading');

        $.ajax({
            url: $bar.data('ajax-url'),
            type: 'post',
            dataType: 'json',
            data: {
                action: 'mec_ymtabs_get_year',
                year: year,
            ...
            success: function (response) {
                ...
                $bar.find('.mec-ymtabs-months').html(response.html);
                $bar.find('.mec-ymtabs-year-label').text(response.year);
                $bar.find('.mec-ymtabs-year-prev').attr('data-mec-year-jump', response.year - 1);
                $bar.find('.mec-ymtabs-year-next').attr('data-mec-year-jump', response.year + 1);
            },
            complete: function () {
                $bar.removeClass('mec-ymtabs-loading');
            }
        });
    }

    function syncFromMonthId($bar, monthId) {
        if (!monthId) return;
        var m = ('' + monthId).match(/^(\d{4})(\d{2})$/);
        if (!m) return;

        var year = parseInt(m[1], 10);
        var month = parseInt(m[2], 10);

        setActiveTab($bar, year, month);

        var shownYear = parseInt($bar.find('.mec-ymtabs-year-label').text(), 10);
        if (shownYear !== year) loadYear($bar, year, year, month);
    }

    /**
     * Navigates the calendar itself to (year, month). Mirrors MEC's own
     * setMonth()/toggleMonth() (monthly-view IIFE above): reuses an
     * already-loaded month in place if present, otherwise fetches it via
     * MEC's own mec_monthly_view_load_month AJAX action — same request,
     * same response shape, same DOM targets MEC's own handler uses — and
     * finishes by triggering load_calendar_data so the tab highlighting
     * (syncFromMonthId, above) and anything else listening for it stay
     * in sync exactly as they would after any native navigation.
     */
    function goToMonth($bar, year, month) {
        var id = $bar.data('mec-id');
        var monthId = '' + year + ('0' + month).slice(-2);
        var $existing = $('#mec_monthly_view_month_' + id + '_' + monthId);

        if ($existing.length) {
            $('#mec_skin_' + id + ' .mec-month-navigator').hide();
            $('#mec_month_navigator_' + id + '_' + monthId).show();
            $('#mec_skin_' + id + ' .mec-month-container').hide().removeClass('mec-month-container-selected');
            $existing.show().addClass('mec-month-container-selected');
            $(document).trigger('load_calendar_data');
            return;
        }

        $bar.addClass('mec-ymtabs-loading');
        var atts = $bar.data('mec-atts') || '';

        $.ajax({
            url: $bar.data('ajax-url'),
            type: 'post',
            dataType: 'json',
            data: 'action=mec_monthly_view_load_month&mec_year=' + year + '&mec_month=' + month + '&' + atts + '&apply_sf_date=0&navigator_click=true',
            success: function (response) {
                if (!response || !response.current_month) return;
                var newId = response.current_month.id;

                $('#mec_skin_events_' + id).append(
                    '<div class="mec-month-container" id="mec_monthly_view_month_' + id + '_' + newId + '" data-month-id="' + newId + '">' + response.month + '</div>'
                );
                $('#mec_skin_' + id + ' .mec-skin-monthly-view-month-navigator-container').append(
                    '<div class="mec-month-navigator" id="mec_month_navigator_' + id + '_' + newId + '">' + response.navigator + '</div>'
                );
                $('#mec_skin_' + id + ' .mec-calendar-events-side').append(
                    '<div class="mec-month-side" id="mec_month_side_' + id + '_' + newId + '">' + response.events_side + '</div>'
                );

                $('#mec_skin_' + id + ' .mec-month-navigator').hide();
                $('#mec_month_navigator_' + id + '_' + newId).show();
                $('#mec_skin_' + id + ' .mec-month-container').hide().removeClass('mec-month-container-selected');
                $('#mec_monthly_view_month_' + id + '_' + newId).show().addClass('mec-month-container-selected');
...
/* =============================================================
 * adventistai.lt customization: month-grid + single-day agenda for
 * Weekly View. Reuses MEC's own "mec_weekly_view_load_month" AJAX
 * action for month navigation (same request MEC's own JS would send)
 * and MEC's own render.php output (already in the page, or in that
 * action's response) for event cards, so this only needs to handle
 * the day-grid/agenda-panel interaction on top.
 * ============================================================= */
(function ($) {
    'use strict';
    ...
    /** Shows the given date's pre-rendered <li> in the agenda panel, hides the rest. */
    function selectDay($root, dateStr) {
        if (!dateStr) return;

        var id = $root.data('mec-id');
        var ymd = dateStr.replace(/-/g, '');

        $root.find('.mec-magenda-day').removeClass('mec-magenda-day-active');
        $root.find('.mec-magenda-day[data-date="' + dateStr + '"]').addClass('mec-magenda-day-active');

        $root.find('.mec-magenda-agenda .mec-weekly-view-date-events').addClass('mec-util-hidden');
        $root.find('#mec_weekly_view_date_events' + id + '_' + ymd).removeClass('mec-util-hidden');
    }

    /** Default selection: today if the displayed month is the real current month, else the 1st. */
    function defaultDayFor($root, year, month) {
        var today = $root.data('today'); // "YYYY-MM-DD", fixed at page load
        var todayYear = parseInt(today.slice(0, 4), 10);
        var todayMonth = parseInt(today.slice(5, 7), 10);
        return (todayYear === year && todayMonth === month) ? today : (year + '-' + pad(month) + '-01');
    }

    /** Builds the 7xN grid of Date objects for a month, Sunday-start. */
    function buildGridDays(year, month, fdow) {
        var first = new Date(year, month - 1, 1);
        var daysInMonth = new Date(year, month, 0).getDate();
        var offset = (first.getDay() - fdow + 7) % 7;
        var totalCells = Math.ceil((offset + daysInMonth) / 7) * 7;

        var days = [];
        var d = new Date(year, month - 1, 1 - offset);
    ...
    function renderGrid($root, year, month, counts) {
        var fdow = parseInt($root.data('fdow'), 10) || 0;
        var today = $root.data('today') || '';
        var days = buildGridDays(year, month, fdow);
        var html = '';

        days.forEach(function (d) {
            var y = d.getFullYear(), m = d.getMonth() + 1, day = d.getDate();
            var dateStr = y + '-' + pad(m) + '-' + pad(day);
            var inMonth = (m === month && y === year);
            var count = counts[dateStr] || 0;
            var cls = 'mec-magenda-day' +
                (!inMonth ? ' mec-magenda-day-pad' : '') +
                (count ? ' mec-magenda-day-has-events' : '') +
                (dateStr === today ? ' mec-magenda-day-today' : '') +
                (d.getDay() === 6 ? ' mec-magenda-day-saturday' : '');

            html += '<button type="button" class="' + cls + '" data-date="' + dateStr + '">' +
                '<span class="mec-magenda-day-num">' + day + '</span>' +
                (count ? '<span class="mec-magenda-day-count">' + count + '</span>' : '') +
                '</button>';
        });

        $root.find('.mec-magenda-grid').html(html);
    }

    /**
     * Today's all-day events show "Šiandien" instead of the usual "All Day"
     * label — timed events (real hour ranges) never match this exact
     * string, so they're untouched. Runs on init and after every agenda
     * refresh, since MEC's own AJAX response has no way to know about
     * this swap ahead of time.
     */
    function relabelToday($root) {
        var allDayLabel = ($root.data('all-day-label') || '').toString().trim();
        var todayWord = $root.data('today-word') || 'Today';
        if (!allDayLabel) return;

        var id = $root.data('mec-id');
        var ymd = (($root.data('today') || '') + '').replace(/-/g, '');
        var $li = $root.find('#mec_weekly_view_date_events' + id + '_' + ymd);
        if (!$li.length) return;

        $li.find('.mec-event-time, .mec-event-detailed-time').each(function () {
            var el = this;
            var textNodes = [];
            ...
            var combined = textNodes.map(function (n) { return n.nodeValue; }).join('').trim();
            if (combined !== allDayLabel) return;

            textNodes[textNodes.length - 1].nodeValue = ' ' + todayWord;
            for (var j = 0; j < textNodes.length - 1; j++) textNodes[j].nodeValue = '';
        });
    }

    /** Pulls {dateStr: count} out of MEC's own returned "month" HTML fragment. */
    function extractCounts(monthHtml) {
        var $tmp = $('<div>').html(monthHtml);
        var counts = {};

        $tmp.find('.mec-weekly-view-date-events').each(function () {
            var m = /_(\d{4})(\d{2})(\d{2})$/.exec(this.id || '');
            if (!m) return;
            var dateStr = m[1] + '-' + m[2] + '-' + m[3];
            // Real event cards carry this date badge; the "No Events"
            // placeholder (same <li>/<article> classes) does not.
            counts[dateStr] = $(this).find('.mec-event-list-weekly-date').length;
        });

        return counts;
    }

    /** Pulls the whole per-day agenda list out of MEC's returned "month" HTML fragment. */
    function extractAgendaList(monthHtml) {
        var $tmp = $('<div>').html(monthHtml);
        var $ul = $tmp.find('.mec-weekly-view-dates-events').first();
        return $ul.length ? $ul.prop('outerHTML') : '';
    }

    function closePickers($root) {
        $root.find('.mec-magenda-picker').attr('hidden', true);
    }

    function loadMonth($root, year, month) {
        month = ((month - 1 + 12) % 12) + 1;
        var atts = $root.data('mec-atts') || '';

        $root.addClass('mec-magenda-loading');

        $.ajax({
            url: $root.data('ajax-url'),
            type: 'post',
            dataType: 'json',
            // Same request MEC's own weekly-view JS sends; the front-end
            // template's own return_items branch answers it unmodified.
            data: 'action=mec_weekly_view_load_month&mec_year=' + year + '&mec_month=' + month + '&' + atts + '&apply_sf_date=1',
            success: function (response) {
                if (!response || typeof response.month === 'undefined') return;

                var counts = extractCounts(response.month);
                $root.find('.mec-magenda-agenda').html(extractAgendaList(response.month));
                renderGrid($root, year, month, counts);

                $root.data('cur-year', year);
                $root.data('cur-month', pad(month));
                $root.find('.mec-magenda-year-label').text(year);
                if (response.current_month && response.current_month.label) {
                    var monthName = response.current_month.label.replace(/^\d{4}\s*/, '');
                    $root.find('.mec-magenda-month-label').text(monthName);
                }
                $root.find('.mec-magenda-picker-month .mec-magenda-picker-item')
                    .removeClass('mec-magenda-picker-item-active')
                    .filter('[data-month="' + pad(month) + '"]')
                    .addClass('mec-magenda-picker-item-active');
                $root.find('.mec-magenda-picker-year .mec-magenda-picker-item')
                    .removeClass('mec-magenda-picker-item-active')
                    .filter('[data-year="' + year + '"]')
                    .addClass('mec-magenda-picker-item-active');

                selectDay($root, defaultDayFor($root, year, month));
                relabelToday($root);
            },
            complete: function () {
                $root.removeClass('mec-magenda-loading');
            }
        });
    }

    $(function () {
        $('.mec-magenda').each(function () {
            var $root = $(this);
        ...
        $(document).on('click', '.mec-magenda-day', function () {
            selectDay($(this).closest('.mec-magenda'), $(this).data('date'));
        });

        $(document).on('click', '.mec-magenda-prev', function () {
            var $root = $(this).closest('.mec-magenda');
            var y = parseInt($root.data('cur-year'), 10);
            var m = parseInt($root.data('cur-month'), 10) - 1;
            if (m < 1) { m = 12; y -= 1; }
            loadMonth($root, y, m);
        });

        $(document).on('click', '.mec-magenda-next', function () {
            var $root = $(this).closest('.mec-magenda');
            var y = parseInt($root.data('cur-year'), 10);
            var m = parseInt($root.data('cur-month'), 10) + 1;
            if (m > 12) { m = 1; y += 1; }
        ...
        $(document).on('click', '.mec-magenda-label', function (e) {
            e.stopPropagation();
            var $root = $(this).closest('.mec-magenda');
            var which = $(this).data('open');
            var $picker = $root.find('.mec-magenda-picker-' + which);
            var wasOpen = $picker.is(':visible');
            closePickers($root);
            if (!wasOpen) $picker.removeAttr('hidden');
        });

        $(document).on('click', '.mec-magenda-picker', function (e) {
            e.stopPropagation();
        });

        $(document).on('click', function () {
            $('.mec-magenda').each(function () { closePickers($(this)); });
        });

        $(document).on('click', '.mec-magenda-picker-month .mec-magenda-picker-item', function () {
            var $root = $(this).closest('.mec-magenda');
            var year = parseInt($root.data('cur-year'), 10);
            closePickers($root);
            loadMonth($root, year, parseInt($(this).data('month'), 10));
        });

        $(document).on('click', '.mec-magenda-picker-year .mec-magenda-picker-item', function () {
            var $root = $(this).closest('.mec-magenda');
